...
<img src="https://upload.wikimedia.org/wikipedia/commons/e/e4/OKX_Logo.svg" width="64" />

A *.NET 6* wrapper for the OKX cryptocurrencies exchange as described on [OKX API docs](https://www.okx.com/docs-v5/en/), including all provided features using clear and readable objects.

Forked from https://github.com/burakoner/OKEx.Net for debugging, exchange rebranding from OKEx to OKX (avoid new users confusion) and update dependencies, for instance, ![migrate JSON serialization from Newtonsoft.Json to .NET 6 System.Text.Json](https://docs.microsoft.com/en-us/dotnet/standard/serialization/system-text-json-migrate-from-newtonsoft-how-to?pivots=dotnet-6-0).

This library is implemented upon the [CryptoExchange.Net project](https://github.com/JKorf/CryptoExchange.Net).

## Installation
Clone this Github repository into your solution and add a reference to your project.

## Getting started
After installing it's time to actually use it. To get started we have to add the OKEx.Net namespace:  `using OkxNet;`.

OkxNet provides two clients to interact with the OKX API. The  `OkxClient`  provides all rest API calls. The  `OkxSocketClient` provides functions to interact with the websocket provided by the OKX API. Both clients are disposable and as such can be used in a `using`statement.

As described on the API docs, WebSocket is an efficient protocol that achieves full-duplex data transmission between client and server, with several advantages over plain HTTP. Developers are recommended to use WebSocket API, specially if they want to retrieve market data and order boook depth.
...
/* Sample Pairs */
var sample_pairs = new List<string> { "BTC-USDT", "ETH-USDT", "ETH-BTC", "XRP-USDT", "BCH-USDT", "EOS-USDT", "OKB-USDT", "XTZ-USDT" };
